...
if (empty($_SESSION['usernamelogin'])) {
    ...
    $sql = "SELECT * FROM Playlists where userid=?";
    $stmt = $pdo->prepare($sql);
    $stmt->execute([$idut]);
    $PlaylistsInfo = $stmt->fetchAll();
    $NORS = $stmt->rowCount();

    if($NORS==0)
        header("Location: ./initial_genre.php");
    else
        $check=0;
...
$stmt = $pdo->prepare($sql);
$stmt->execute([ $stmtID,$_SESSION['userIDlogin'], $namePlaylist, date("Y/m/d"),$typePlaylist,Null,date("Y/m/d")]);
...
<div class="sidebar bg-dark" id="sidebar" style="padding-top: 35px;">
    <a style="font-size: 20px;" href="./index.php"><i class="fas fa-home"></i> Home</a>
    <a style="font-size: 20px;" href="./search.php"><i class="fas fa-search"></i> Search</a>
    <a style="font-size: 20px;" href="./playlists.php"><i class="fas fa-list-music"></i> Your Library</a>
    ...
    ?>
        <div class="col-lg-auto">
            ...
            <div class="a ih-item square colored effect6 mrgn-top ">
                <a href="#">
                    <div class="img img-fluid"><img src="./images/m.jpeg" name="<?php echo $Playlist['Name']; ?>" alt="img"></div>
                    <div class="info">

                        <p>
                        <h2 style="color:rgb(210,0,0);"><?php
                         echo $Playlist['Name'];
                          ?></h2>
