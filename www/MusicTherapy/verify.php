...
session_start();
if (empty($_SESSION['usernameloginV']))
  $userN = $_SESSION['username'];
else
  $userN = $_SESSION['usernameloginV'];

$sql = "SELECT Email,VCode FROM UserTable Where UserName=?;";
...
if (isset($_POST['Verify'])) {
  ...
  $sql = "SELECT VCode FROM UserTable Where UserName=?;";
  $stmt = $pdo->prepare($sql);
  $stmt->execute([$userN]);
  $result = $stmt->fetchAll();
  ...
  if ($codeV == $userCode) {
    $sql = "UPDATE UserTable SET VCode=-1  Where UserName=?;";
    $stmt = $pdo->prepare($sql);
    $stmt->execute([$userN]);
...
<body>
  ...
  <div class="container">
    ...
    <form method="post" action="./verify.php">
      <?php
      if (!empty($_SESSION['usernameloginV']))
        echo '<h5 class="text text-danger" style="text-align: center;margin-top:50px; font-size: 30px">Your Account not verify, please Verify Your Account </h5>';
      ?>
